...
fetch('https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics%2Cplayer&id='+video_id+'&key=AIzaSyAYmKOWgNCWc41-rsMnU0objce1Ukn4EmE')
.then((res) => {
  return res.json();
}).then((data) => {
...
function getSuggestions() {
  fetch('https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=15&relatedToVideoId='+video_id+'&type=video&key=AIzaSyAYmKOWgNCWc41-rsMnU0objce1Ukn4EmE')
  .then((res) => {
    return res.json();
  }).then((data) => {
    // console.log(data);
